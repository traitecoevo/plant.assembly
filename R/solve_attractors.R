...
viable_fitness <- function(trait, p, bounds=NULL, value=NULL,
  ...
  if (is.null(bounds)) {
    if (length(trait) == 1L) {
      warning("Please provide real bounds")
      if (log_scale) {
        bounds <- c(1e-5, 1e3)
      } else {
        bounds <- c(-Inf, Inf)
      ...
      stop("You must provide bounds")
    }
  }
  if (is.null(value)) {
    value <- rbind(unlist(p$strategy_default$parameters[trait]),
                   deparse.level=0)
  }
  if (!is.matrix(bounds)) {
    warning("Non matrix bounds provided")
    bounds <- rbind(bounds, deparse.level=0)
  }
  lower <- bounds[,1]
  upper <- bounds[,2]

  if (any(value > upper | value < lower)) {
  ...
  if (length(trait) == 1L) {
    out <- positive(f, value, dx, lower=lower, upper=upper)
    bounds <- rbind(x=out)
  } else {
    ...
    out <- positive2(f, value, lower=lower, upper=upper)
    bounds <- t(apply(out$points[out$labels,], 2, range))
  }
  if (log_scale) {
    bounds <- exp(bounds)
  }
...
positive_bracket <- function(f, x, dx, lower=-Inf, upper=Inf, factor=2) {
  fx <- f(x)
  if (fx < 0) {
