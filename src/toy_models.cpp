...
//     W = (R/x') s(x') g          (returns log W; resident value ~0)
//   resident equilibrium: W_i = 1 for every resident; single resident solves
//     (R/x) s(x) (1 - exp(-N))/N = 1   (exists iff (R/x) s(x) > 1).
//
// Only the products alpha*R and beta*R matter. Distinct from the Geritz 1998
// soft-selection model (geritz_*). No closed-form singular strategy (numerical);
// branching diagnosed by the sign of the invasion-fitness second derivative.
// This is the model implemented in Daniel's MATLAB (Offspring-SmithFretwell).
...
//' @keywords internal
// [[Rcpp::export]]
NumericVector geritz99_log_fitness(NumericVector x_mut, NumericVector x_res,
                                   NumericVector n_res, List pars) {
  ...
  for (int j = 0; j < nr; j++) { cres[j] = geritz99::c_of(x_res[j], alpha); N[j] = n_res[j]; }
  for (int i = 0; i < nm; i++) {
    double mp = x_mut[i];
    double s = geritz99::s_of(mp, beta);
    if (s <= 0.0 || mp <= 0.0) { out[i] = R_NegInf; continue; }
    double seeds = R / mp * s;
    double g = (nr == 0) ? 1.0 : geritz99::estab(geritz99::c_of(mp, alpha), cres, N);
...
//' @keywords internal
// [[Rcpp::export]]
NumericVector geritz99_equilibrium(NumericVector x_res, List pars,
  ...
  if (nr == 0) return n;
  if (nr == 1) {
    double m = x_res[0], s = geritz99::s_of(m, beta);
  ...
  for (int j = 0; j < nr; j++) {
    cres[j] = geritz99::c_of(x_res[j], alpha);
    NumericVector xj(1); xj[0] = x_res[j];
    n[j] = geritz99_equilibrium(xj, pars)[0];
    if (n[j] <= 0.0) n[j] = 0.01;
  }
  for (int it = 0; it < max_iter; it++) {
    ...
    NumericVector nn(nr);
    for (int i = 0; i < nr; i++) {
      double m = x_res[i], s = geritz99::s_of(m, beta);
      double seeds = (s > 0.0 && m > 0.0) ? R / m * s : 0.0;
      double W = seeds * geritz99::estab(cres[i], cres, Nv);
      double newN = n[i] * W;
