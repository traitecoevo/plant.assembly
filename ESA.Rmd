...
landscapes <- 
  obj$history %>% 
  purrr:::map_df(~.x$fitness_approximate_points %>% as_tibble(), .id = "step") %>% mutate(trait_value =lma) %>%
...
f_extract_residents <- function(.x) {
    ...
    tibble(
      ...
      fitness = 
        approx(
          .x$fitness_approximate_points[,"lma"], 
          .x$fitness_approximate_points[, "fitness"], 
          .x$traits[, 1]
            )$y
  )
} 
...
fitness1 <- 
  community$fitness_approximate_points %>% as_tibble() %>% mutate(step=1, trait_value=lma) 
...
fitness3 <- community3b$fitness_approximate_points %>% as_tibble() %>% mutate(step = 1,trait_value=lma)
...
fitness4 <- community4$fitness_approximate_points %>% as_tibble() %>% mutate(step = 1,trait_value=lma)
