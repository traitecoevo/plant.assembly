...
test_that("geritz99 resident at equilibrium has log fitness zero", {
  pars <- list(R = 1, alpha = 7, beta = 15)
  for (m in c(0.1, 0.3, 0.645, 0.9)) {
    N <- geritz99_equilibrium(m, pars)
    expect_gt(N, 0)
    expect_equal(geritz99_log_fitness(m, m, N, pars), 0, tolerance = 1e-10)
...
test_that("geritz99 self-competition matches the closed form (1-e^-N)/N", {
  pars <- list(R = 1, alpha = 4.5, beta = 15)
  # W(m,m) = (R/m) s(m) g_res(N); so g_res(N) = W / ((R/m) s(m)).
  m <- 0.3; N <- 2.5
  s <- 1 - 2 * exp(-pars$beta * m)
  logW <- geritz99_log_fitness(m, m, N, pars)
  g_res <- exp(logW) / ((pars$R / m) * s)
  expect_equal(g_res, (1 - exp(-N)) / N, tolerance = 1e-8)
})

test_that("geritz99 non-viable seed sizes have zero equilibrium density", {
  pars <- list(R = 1, alpha = 6, beta = 15)
  # just above the minimum viable size m_min = ln2/beta, (R/m)s(m) < 1
  m_min <- log(2) / pars$beta
  expect_equal(geritz99_equilibrium(m_min * 1.005, pars), 0, tolerance = 1e-12)
  # log fitness is -Inf below m_min (s = 0)
  expect_equal(geritz99_log_fitness(m_min * 0.5, 0.3,
                                    geritz99_equilibrium(0.3, pars), pars),
               -Inf)
})

test_that("geritz99 is a CSS at low asymmetry (alpha*R = 4.5, beta*R = 15)", {
  h <- harness_geritz99(alpha = 4.5, beta = 15)
...
test_that("geritz99 branches at high asymmetry (alpha*R = 7, beta*R = 15)", {
  h <- harness_geritz99(alpha = 7, beta = 15)
...
test_that("geritz99 depends only on the products alpha*R and beta*R", {
  # same alpha*R, beta*R via different R should give the same singular strategy
  h1 <- harness_geritz99(alpha = 7, beta = 15, R = 1)
  h2 <- harness_geritz99(alpha = 3.5, beta = 7.5, R = 2)
